"""
Dashboard Schemas - Pydantic models for dashboard statistics.
"""
...
class SeverityStatsResponse(BaseModel):
    distribution: Dict[str, int] = Field(description="Thống kê theo mức độ nghiêm trọng")
    total: int = Field(description="Tổng số phát hiện")

    class Config:
        json_schema_extra = {
            "example": {
                "distribution": {"mild": 60, "moderate": 30, "severe": 10},
                "total": 1250
            }
        }
