...
class AuditRepository(BaseRepository[AuditLog]):
    ...
    async def get_audit_logs(
        ...
        if role_name:
            # Subquery to find user_ids with specific role
            subquery = select(UserRole.user_id).join(
                Role).where(Role.role_name == role_name)
            stmt = stmt.where(AuditLog.user_id.in_(subquery))

        # Count total
        # We need to count the filtered query
        # Use select(func.count()).select_from(stmt.subquery()) to handle joins correctly if needed
        # But here straightforward counting on the join might be cleaner.
        # Actually count query usually duplicates filters but simplifies select.
        ...
        for row in rows:
            ...
            if audit_log.user_id:
                roles_result = await self.db.execute(
                    ...
                    .where(UserRole.user_id == audit_log.user_id)
                )
