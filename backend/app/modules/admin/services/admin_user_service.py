...
from app.modules.admin.repository.admin_user_repository import AdminUserRepository
...
# Keep shared exceptions for general validation if needed, or stick to Admin-specific ones.
# The previous code used shared exceptions. The requirement is to create exceptions.
# I will use Admin* errors primarily, but for simple "Bad Request" on input, shared is fine.
# User mentioned "exception chưa được tạo".
...
class AdminUserService:
    ...
    async def create_user(
        ...
        from app.utils.logging_utils import sanitize_user_data
        ...
        try:
            ...
            # Refresh to get the user object if needed, or just rely on ID for detail fetch
            # Since get_user_detail fetches fresh, we don't strictly need to refresh new_user object here
            # unless we need it for email sending (which needs email address)
            # Let's fetch the user object for email sending
            ...
            await self.audit_service.log_action(
                ...
                admin_role=self._get_admin_role(current_admin),
                action="CREATE_USER",
                resource_type="user",
            ...
            return await self.get_user_detail(new_user_id)

        except (ConflictError, BadRequestError, AdminUserNotFoundError) as e:
            ...
            await self.audit_service.log_action(
                ...
                action="CREATE_USER",
                resource_type="user",
                description=f"Failed to create user {user_data.email}",
                ip_address=ip_address,
                user_agent=user_agent,
                status="failed",
                error_message=str(e)
            ...
            await self.audit_service.log_action(
                ...
                action="CREATE_USER",
                resource_type="user",
                description=f"Error creating user {user_data.email}",
                ip_address=ip_address,
                user_agent=user_agent,
                status="error",
                error_message=str(e)
            )
            raise AdminActionFailedError(
        ...
        if await self.repository.username_exists(username):
            ...
            if existing:
                if not existing.is_deleted:
                    ...
                    # Rename deleted user to free up username
                    # Ensure we don't rename the same user twice if they matched both email and username
    ...
    async def _rename_deleted_user(self, user: User) -> None:
        ...
        # Check if already renamed to avoid double suffix
    ...
    async def update_user(
        ...
        if not user or user.is_deleted:
            raise AdminUserNotFoundError(message=f"User {user_id} not found")
        ...
        try:
        ...
        except (ConflictError, AdminUserNotFoundError) as e:
            await self.db.rollback()
            raise
        except Exception as e:
            await self.db.rollback()
            ...
            await self.audit_service.log_action(
                ...
                resource_type="user",
                resource_id=str(user_id),
                description=f"Error updating user {user_id}",
                ip_address=ip_address,
                user_agent=user_agent,
                status="error",
                error_message=str(e)
            )
            raise AdminActionFailedError(message=f"Update failed: {str(e)}")
    ...
    async def delete_user(
        ...
        try:
            ...
            await self.audit_service.log_action(
                ...
                resource_type="user",
                resource_id=str(user_id),
                description=f"Soft deleted user {user_id}",
                ip_address=ip_address,
                user_agent=user_agent,
                status="success"
            )
            ...
            raise AdminActionFailedError(message=f"Delete failed: {str(e)}")
    ...
    async def update_user_status(
        ...
        try:
            ...
            await self.audit_service.log_action(
                ...
                resource_type="user",
                resource_id=str(user_id),
                description=f"Changed user status to {status_text}",
            ...
            raise AdminActionFailedError(message="Status update failed")
    ...
    async def resend_verification_email(
        ...
        try:
            ...
            await self.audit_service.log_action(
                ...
                resource_type="user",
                resource_id=str(user_id),
                description=f"Resent verification email to user {user_id}",
                ip_address=ip_address,
                user_agent=user_agent,
                status="success"
            )
