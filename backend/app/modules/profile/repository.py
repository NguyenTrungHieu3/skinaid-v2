"""
Profile Repository — Data access layer for UserProfile.

Sử dụng SQLModel/SQLAlchemy ORM thay vì raw SQL.
"""
...
class ProfileRepository(BaseRepository[UserProfile]):
    ...
    async def search(
        ...
        if full_name:
            stmt = stmt.where(UserProfile.full_name.ilike(
                f"%{full_name}%"))  # type: ignore

        if gender:
            stmt = stmt.where(UserProfile.gender == gender)

        # Filter by age (calculated from date_of_birth)
        # Note: SQLModel/SQLAlchemy query construction for age calculation
        # Filter by age (calculated from date_of_birth)
        ...
        stmt = stmt.offset(skip).limit(limit).order_by(
            UserProfile.updated_at.desc())  # type: ignore
    ...
    async def get_statistics(self) -> dict[str, Any]:
        ...
        # 1. Total users (from User table)
        total_users_stmt = select(func.count()).select_from(User)
        total_result = await self.session.exec(total_users_stmt)
        total_users = total_result.one()

        # 2. Users with profile
        with_profile_stmt = select(func.count(UserProfile.user_id))
        profile_result = await self.session.exec(with_profile_stmt)
        users_with_profile = profile_result.one()

        # 3. Complete profiles
        complete_stmt = select(func.count()).where(
            UserProfile.full_name.is_not(None),  # type: ignore
            UserProfile.phone.is_not(None),  # type: ignore
            UserProfile.date_of_birth.is_not(None),  # type: ignore
            UserProfile.gender.is_not(None),  # type: ignore
        )
        complete_result = await self.session.exec(complete_stmt)
        complete_profiles = complete_result.one()

        # 4. Gender distribution
        gender_stmt = (
            select(UserProfile.gender, func.count(UserProfile.user_id))
            .where(UserProfile.gender.is_not(None))  # type: ignore
            .group_by(UserProfile.gender)
        )
        gender_result = await self.session.exec(gender_stmt)
        gender_dist = {row[0]: row[1] for row in gender_result}
