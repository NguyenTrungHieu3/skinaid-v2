...
from app.modules.auth.services.token_family_service import (
    create_token_family, check_token_family_revoked, revoke_token_family, revoke_entire_chain
)
...
class AuthController:
    ...
    async def refresh_token(self, refresh_request) -> Union[SuccessResponse[TokenResponse], ErrorResponse]:
        try:
            ...
            if await check_token_family_revoked(self.db, old_jti):
                logger.error("[REFRESH_TOKEN] REUSE DETECTED user %s jti: %s", user_id, old_jti)
                await revoke_entire_chain(self.db, old_jti)
                return ErrorResponse(
            ...
            await revoke_token_family(self.db, old_jti)
