...
class FirstAidController:
    ...
    async def get_first_aid_guide(
        ...
        try:
            logger.info(f"[FIRSTAID_GUIDE] Getting guide for: {wound_type}/{severity}, sub_type: {sub_type}")
            ...
            if not guide:
                ...
                if sub_type:
                    error_message = Message.FIRSTAID_GUIDE_NOT_FOUND_WITH_SUBTYPE_MSG.format(
            ...
            if sub_type:
                success_message = f"{Message.FIRSTAID_GUIDE_FOUND_FOR_MSG.format(wound_type=wound_type, severity=severity)} với sub_type '{sub_type}'"
            else:
    ...
    async def get_available_wound_types(
        ...
        try:
            ...
            logger.info(f"[WOUND_TYPES] Success: {len(wound_type_responses)} types found")
    ...
    async def search_first_aid_guides(
        ...
        try:
            ...
            logger.info(f"[SEARCH_GUIDES] Success: {len(guide_responses)} guides found")
    ...
    async def validate_guide_availability(
        ...
        try:
            ...
            if guide:
                # Format success message based on sub_type presence
                if sub_type:
                    success_message = Message.FIRSTAID_GUIDE_AVAILABLE_WITH_SUBTYPE_MSG.format(
                        wound_type=wound_type,
                        severity=severity,
                        sub_type=sub_type
                    )
                else:
                    success_message = Message.FIRSTAID_GUIDE_AVAILABLE_FOR_MSG.format(
                        wound_type=wound_type,
                        severity=severity
                    )
                ...
                if sub_type:
                    message = Message.FIRSTAID_GUIDE_NOT_AVAILABLE_WITH_SUBTYPE_MSG.format(
                        wound_type=wound_type,
                        severity=severity,
                        sub_type=sub_type
                    )
                else:
                    message = Message.FIRSTAID_GUIDE_NOT_AVAILABLE_FOR_MSG.format(
                        wound_type=wound_type,
                        severity=severity
                    )
    ...
    async def _get_available_combinations(self) -> List[str]:
        ...
        try:
            ...
            logger.warning(f"[COMBINATIONS] Failed to get combinations: {e}")
            return []