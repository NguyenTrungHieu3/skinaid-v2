...
class ResponseMapper:
    ...
    @staticmethod
    def map_wound_analysis(analysis:Any, include_detections:bool =True, include_firstaid_details:bool =True) -> Dict:
        response ={
        ...
        if include_detections:
            ...
            if wound_detections_attr.loaded_value is not NO_VALUE:
                response['significant_wounds'] = [
                    ResponseMapper.map_wound_detection(d, include_firstaid_details)
    ...
    def map_wound_analysis_basic(analysis: Any) -> Dict:
        """Basic response for POST /ai/analyze - minimal information"""
        return {
            'analysis_id': str(analysis.analysis_id),
            'image_url': analysis.image_url,
            'file_name': analysis.file_name,
    ...
    @staticmethod
    def map_wound_detection(detection: Any, include_firstaid_details: bool = True) -> Dict:
        base_response = {
            ...
            'firstaid_guide_id': str(detection.firstaidguide_id) if detection.firstaidguide_id else None,
        }
    ...
    @staticmethod
    def error_response(error: str, code: Optional[str] = None) -> Dict:
