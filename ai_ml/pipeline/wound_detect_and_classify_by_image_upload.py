...
yolo_model_path = str(BASE_DIR/"detection/models/model_2_class_v1.pt")
efficientnet_model_path = str(BASE_DIR/"classification/models/final_model.pth")
# Load model YOLO
...
def detect_wounds(image, conf_threshold=0.25):
    results = model_yolo.predict(source=image, conf=conf_threshold, save=False)
    detections = []
...
def detect_and_classify(image, conf_threshold=0.25):
    detected_wounds = detect_wounds(image, conf_threshold)
    boxes = []
    for detected_wound in detected_wounds:
        boxes.append(detected_wound["bbox"])
    cropped_images = crop_wounds(image, boxes)
